...
func main() {
	...
	list := strings.Split(ret.Val(), "\n")
	l = len(list)
	//p, _ := regexp.Compile(`\d{1,3}\.){3}\d{1,3}:\d{1,5}`)
	//p, _ := regexp.Compile(`\d{1,3}\.){3}\d{1,3}`)
	var masters string
	sep := ""
	p, _ := regexp.Compile(`(\d{1,3}\.){3}\d{1,3}:\d{1,5}`)
	if p == nil {
		log.Fatal("Regexp compile failed")
	}
	for i := 0; i < l; i++ {
		if strings.Contains(list[i], "master") {
			masters += sep
			masters += p.FindString(list[i])
			sep = " "
		}
	}
	fmt.Println(masters)
